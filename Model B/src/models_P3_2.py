...
class ImageWiseModel(BaseModel):
    def __init__(self, args, image_wise_network, patch_wise_network):
        super(ImageWiseModel, self).__init__(args, image_wise_network, args.checkpoints_path + '/weightsP3_2_' + image_wise_network.name() + '.pth')
    ...
    def train(self):
        ...
        print('Start training image-wise network (P3_2): {}\n'.format(time.strftime('%Y/%m/%d %H:%M')))
    ...
    def test(self, path, verbose=True, ensemble= True):
        ...
        if verbose:
            print('P3_2 Context-aware Patch-wise Scale 896 x 672')
        ...
        for index, (image, file_name) in enumerate(data_loader):
            ...
            for i in range(n_bins):
                ...
                newPatch = np.concatenate((np.array(patches[i][4]), np.array(patches[i][0]),
                                           np.array(patches[i][1])))
                newPatch1 = np.concatenate((np.array(patches[i][4]), np.array(patches[i][5]),
                                           np.array(patches[i][1])))
                newPatch2 = np.concatenate((np.array(patches[i][4]), np.array(patches[i][8]),
                                           np.array(patches[i][9])))
                newPatch3 = np.concatenate((np.array(patches[i][4]), np.array(patches[i][5]),
                                           np.array(patches[i][9])))
                newPatch4 = np.concatenate((np.array(patches[i][2]), np.array(patches[i][3]),
                                           np.array(patches[i][7])))
                newPatch5 = np.concatenate((np.array(patches[i][2]), np.array(patches[i][6]),
                                           np.array(patches[i][7])))
                newPatch6 = np.concatenate((np.array(patches[i][10]), np.array(patches[i][6]),
                                           np.array(patches[i][7])))
                newPatch7 = np.concatenate((np.array(patches[i][10]), np.array(patches[i][11]),
                                           np.array(patches[i][7])))
        ...
        if self.args.debug and augment and os.path.exists(images_path):
            ...
            for index, (images, labels) in enumerate(output_loader):
                ...
                for i in range(bsize):
                    single_Image = []                    
                    newPatch = np.concatenate((np.array(res[i][4]), np.array(res[i][0]),
                                               np.array(res[i][1])))
                    newPatch1 = np.concatenate((np.array(res[i][4]), np.array(res[i][5]),
                                               np.array(res[i][1])))
                    newPatch2 = np.concatenate((np.array(res[i][4]), np.array(res[i][8]),
                                               np.array(res[i][9])))
                    newPatch3 = np.concatenate((np.array(res[i][4]), np.array(res[i][5]),
                                               np.array(res[i][9])))
                    newPatch4 = np.concatenate((np.array(res[i][2]), np.array(res[i][3]),
                                               np.array(res[i][7])))
                    newPatch5 = np.concatenate((np.array(res[i][2]), np.array(res[i][6]),
                                               np.array(res[i][7])))
                    newPatch6 = np.concatenate((np.array(res[i][10]), np.array(res[i][6]),
                                               np.array(res[i][7])))
                    newPatch7 = np.concatenate((np.array(res[i][10]), np.array(res[i][11]),
                                               np.array(res[i][7])))
