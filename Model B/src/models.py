...
class PatchWiseModel(BaseModel):
    ...
    def test(self, path, verbose=True):
        ...
        for index, (image, file_name) in enumerate(data_loader):
            ...
            preds.append(ntpath.basename(file_name[0]))
            preds.append(output)
            _, predicted = torch.max(output.data, 1)
...
class ImageWiseModel(BaseModel):
    ...
    def validate1(self, verbose=True, roc=False):
        self.network.eval()
        
        if self._test_loader is None:
            with torch.no_grad():
                self._test_loader = self._patch_loader1(self.args.dataset_path + TRAIN_PATH, False)
    ...
    def test(self, path, verbose=True, ensemble= True):
        ...
        if verbose:
            print('Context-aware DenseNet Scale II')
        ...
        for index, (image, file_name) in enumerate(data_loader):
            ...
            """#Context aware of neighbour patches
            row = int(((336 - PATCH_SIZE)/ self.args.patch_stride)+1)
            col = int(((448 - PATCH_SIZE)/ self.args.patch_stride)+1)
            #print(row,col)
            PATCH_INDICES = np.zeros((row,col))
            
            ptch_idx = 0
            for i in range(row):
                for j in range(col):
                    PATCH_INDICES[i][j] = ptch_idx
                    ptch_idx = ptch_idx+1
            ...
            for i in range(row):
                for j in range(col):
                    neighbours_for_index = []
    
                    if (i-1 >= 0 and  i-1 < row) and  (j >= 0 and  j < col):
                        neighbours_for_index.append(PATCH_INDICES[i-1][j])
                        
                    if (i+1 >= 0 and  i+1 < row) and  (j >= 0 and  j < col):
                        neighbours_for_index.append(PATCH_INDICES[i+1][j])
                        
                    if (i >= 0 and  i < row) and  (j+1 >= 0 and  j+1 < col):
                        neighbours_for_index.append(PATCH_INDICES[i][j+1])
                        
                    if (i >= 0 and  i < row) and  (j-1 >= 0 and  j-1 < col):
                        neighbours_for_index.append(PATCH_INDICES[i][j-1])
                        
                    if (i-1 >= 0 and  i-1 < row) and  (j-1 >= 0 and  j-1 < col):
                        neighbours_for_index.append(PATCH_INDICES[i-1][j-1])
                    ...
                    Total_neigh.append(neighbours_for_index)"""

            """separate = []
            for u in range(len(Total_neigh)):
                for k in range(len(Total_neigh[u])):
                    separate.extend((u,Total_neigh[u][k]))
            
            i=0
            new_list=[]
            while i<len(separate):
                new_list.append(separate[i:i+2])
                i+=2
            
            data_new = {tuple(item) for item in map(sorted, new_list)}
         """       
            ...
            for i in range(n_bins):
                ...
                newPatch = (np.array(patches[i][0]) + np.array(patches[i][1]) + 
                                np.array(patches[i][3]) + np.array(patches[i][4])) / 4
                    
                newPatch1 = (np.array(patches[i][1]) + np.array(patches[i][0]) + 
                                np.array(patches[i][3]) + np.array(patches[i][4]) + 
                                np.array(patches[i][2]) + np.array(patches[i][5])) / 6
                    
                    
                newPatch2 = (np.array(patches[i][2]) + np.array(patches[i][1]) +
                                 np.array(patches[i][4]) + np.array(patches[i][5]))/4
            ...
            res.append([new_maj, confidence, ntpath.basename(file_name[0])])
...
"""
        ...
        if self.args.debug and augment and os.path.exists(images_path):
            ...
            for index, (images, labels) in enumerate(output_loader):
                ...
                for i in range(bsize):
                    single_Image = []                    
                    newPatch = (np.array(res[i][0]) + np.array(res[i][1]) + 
                                np.array(res[i][3]) + np.array(res[i][4])) / 4
                    
                    newPatch1 = (np.array(res[i][1]) + np.array(res[i][0]) + 
                                np.array(res[i][3]) + np.array(res[i][4]) + 
                                np.array(res[i][2]) + np.array(res[i][5])) / 6
                    
                    
                    newPatch2 = (np.array(res[i][2]) + np.array(res[i][1]) +
                                 np.array(res[i][4]) + np.array(res[i][5]))/4
    ...
    def _patch_loader1(self, path, augment):
        images_path = '{}/{}_images.npy'.format(self.args.checkpoints_path, self.network.name())
        labels_path = '{}/{}_labels.npy'.format(self.args.checkpoints_path, self.network.name())
        ...
        if self.args.debug and augment and os.path.exists(images_path):
            ...
            for index, (images, labels) in enumerate(output_loader):
                ...
                res = self.patch_wise_model.output1(images.view((-1, 3, 224, 224)))
        ...
        your_file = open(r'C:\Users\Quadro\Dense 448 x 336 Training_data_patch56.txt', 'ab')
        for t in output_images:
            np.savetxt(your_file, t, delimiter = '\n')
        your_file.close() 
        
        #print(output_images)
        #print(output_labels)
        images, labels = torch.stack(output_images), torch.stack(output_labels).squeeze()
        print(labels)
