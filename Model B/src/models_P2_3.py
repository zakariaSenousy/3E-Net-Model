...
class PatchWiseModel(BaseModel):
    ...
    def test(self, path, verbose=True):
        ...
        if verbose:
            print('Finalising Patch-wise Scale 896 x 672')
        ...
        for index, (image, file_name) in enumerate(data_loader):
            ...
            new_maj = np.divide((np.sum(np.eye(3)[np.array(predicted).reshape(-1)], axis=0)),len(output)) * 100
            #print(new_maj)
...
class ImageWiseModel(BaseModel):
    def __init__(self, args, image_wise_network, patch_wise_network):
        super(ImageWiseModel, self).__init__(args, image_wise_network, args.checkpoints_path + '/weightsP2_3_' + image_wise_network.name() + '.pth')
    ...
    def train(self):
        ...
        print('Start training image-wise network (P2_3): {}\n'.format(time.strftime('%Y/%m/%d %H:%M')))
    ...
    def test(self, path, verbose=True, ensemble= True):
        ...
        if verbose:
            print('P2_3 Context-aware Patch-wise Scale 896 x 672')
        ...
        for index, (image, file_name) in enumerate(data_loader):
            ...
            with torch.no_grad():
                ...
                patches = patches.view(n_bins, -1, 28, 28)
            ...
            for i in range(n_bins):
                ...
                #newPatch1 = np.concatenate((np.array(patches[i][1]), np.array(patches[i][5])))
                newPatch2 = np.concatenate((np.array(patches[i][5]), np.array(patches[i][6])))
                newPatch3 = np.concatenate((np.array(patches[i][4]), np.array(patches[i][8])))
                newPatch4 = np.concatenate((np.array(patches[i][8]), np.array(patches[i][9])))
                newPatch5 = np.concatenate((np.array(patches[i][6]), np.array(patches[i][10])))
                #newPatch6 = np.concatenate((np.array(patches[i][10]), np.array(patches[i][11])))
                #newPatch7 = np.concatenate((np.array(patches[i][2]), np.array(patches[i][3])))
                newPatch8 = np.concatenate((np.array(patches[i][3]), np.array(patches[i][7])))
                #newPatch9 = np.concatenate((np.array(patches[i][0]), np.array(patches[i][5])))
                newPatch10 = np.concatenate((np.array(patches[i][1]), np.array(patches[i][6])))
                newPatch11 = np.concatenate((np.array(patches[i][2]), np.array(patches[i][7])))
                newPatch12 = np.concatenate((np.array(patches[i][4]), np.array(patches[i][9])))
                newPatch13 = np.concatenate((np.array(patches[i][5]), np.array(patches[i][10])))
                newPatch14 = np.concatenate((np.array(patches[i][6]), np.array(patches[i][11])))
                ...
                Single_test_image.append(newPatch8)
                #Single_test_image.append(newPatch9)
                Single_test_image.append(newPatch10)
                Single_test_image.append(newPatch11)
                Single_test_image.append(newPatch12)
                Single_test_image.append(newPatch13)
                Single_test_image.append(newPatch14)
            ...
            res.append([prob, confidence, ntpath.basename(file_name[0])])
    ...
    def _patch_loader(self, path, augment):
        ...
        if self.args.debug and augment and os.path.exists(images_path):
            ...
            for index, (images, labels) in enumerate(output_loader):
                ...
                res = res.view((bsize, -1, 28, 28)).data.cpu().numpy()
                ...
                for i in range(bsize):
                    ...
                    #newPatch1 = np.concatenate((np.array(res[i][1]), np.array(res[i][5])))
                    newPatch2 = np.concatenate((np.array(res[i][5]), np.array(res[i][6])))
                    newPatch3 = np.concatenate((np.array(res[i][4]), np.array(res[i][8])))
                    newPatch4 = np.concatenate((np.array(res[i][8]), np.array(res[i][9])))
                    newPatch5 = np.concatenate((np.array(res[i][6]), np.array(res[i][10])))
                    #newPatch6 = np.concatenate((np.array(res[i][10]), np.array(res[i][11])))
                    #newPatch7 = np.concatenate((np.array(res[i][2]), np.array(res[i][3])))
                    newPatch8 = np.concatenate((np.array(res[i][3]), np.array(res[i][7])))
                    #newPatch9 = np.concatenate((np.array(res[i][0]), np.array(res[i][5])))
                    newPatch10 = np.concatenate((np.array(res[i][1]), np.array(res[i][6])))
                    newPatch11 = np.concatenate((np.array(res[i][2]), np.array(res[i][7])))
                    newPatch12 = np.concatenate((np.array(res[i][4]), np.array(res[i][9])))
                    newPatch13 = np.concatenate((np.array(res[i][5]), np.array(res[i][10])))
                    newPatch14 = np.concatenate((np.array(res[i][6]), np.array(res[i][11])))
                    ...
                    single_Image.append(newPatch8)
                    #single_Image.append(newPatch9)
                    single_Image.append(newPatch10)
                    single_Image.append(newPatch11)
                    single_Image.append(newPatch12)
                    single_Image.append(newPatch13)
                    single_Image.append(newPatch14)
