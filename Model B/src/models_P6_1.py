...
class ImageWiseModel(BaseModel):
    def __init__(self, args, image_wise_network, patch_wise_network):
        super(ImageWiseModel, self).__init__(args, image_wise_network, args.checkpoints_path + '/weightsP6_1_' + image_wise_network.name() + '.pth')
    ...
    def train(self):
        ...
        print('Start training image-wise network (P6_1): {}\n'.format(time.strftime('%Y/%m/%d %H:%M')))
    ...
    def test(self, path, verbose=True, ensemble= True):
        ...
        if verbose:
            print('P6_1 Context-aware Patch-wise Scale 896 x 672')
        ...
        for index, (image, file_name) in enumerate(data_loader):
            ...
            for i in range(n_bins):
                ...
                newPatch3 = np.concatenate((np.array(patches[i][5]), np.array(patches[i][6]),
                                           np.array(patches[i][7]), np.array(patches[i][11]),
        ...
        if self.args.debug and augment and os.path.exists(images_path):
            ...
            for index, (images, labels) in enumerate(output_loader):
                ...
                for i in range(bsize):
                    ...
                    newPatch3 = np.concatenate((np.array(res[i][5]), np.array(res[i][6]),
                                               np.array(res[i][7]), np.array(res[i][11]),
